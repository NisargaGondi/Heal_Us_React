...
export default function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch('http://localhost:3002/loggedin', {
        method: 'POST',
        headers: {
  ...
  return (
    ...
    <div className="login-wrapper d-flex justify-content-center">
        ...
        <div className="input-group">
            <input type="text" name="userInput input-text" value={usn} id="userInput" pattern="1BM\d{2}[a-zA-Z]{2}\d{3}"  data-placeholder="1BM12AB012" maxLength="10" onChange={handleUsnChange} required />
        <label htmlFor="input-text" style={{color: 'black'}}>USN (University Student Number)</label>
        ...
        <input type="password" name="loginPassword" id="Password" minLength="8" value={password} onChange={handlePasswordChange} required/>
        <label htmlFor="loginPassword"style={{color: 'black'}}>Password</label>
        ...
        <br/><br/>
        <table><tbody>
          <tr><td><NavLink to="/ForgotPassword"><h5 className="fs-2">Forgot password?</h5></NavLink></td>
        <td>
          <input type="submit" onClick={handleSubmit} value="Login" style={{marginLeft:115+'px', color:'white'}} className="submit-btn text-center" /></td>
      </tr>
      <tr>
